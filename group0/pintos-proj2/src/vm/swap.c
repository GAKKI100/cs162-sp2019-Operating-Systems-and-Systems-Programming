...
vm_swap_init(void){
     ...
     * each single bit of 'swap_available' correspons to a block region, 
...
vm_swap_out(void *page){
    ...
    for(i = 0; i < SECTORS_PER_PAGE; i++){
        block_write(swap_block, 
            ...
            /* targe address */ page + (BLOCK_SECTOR_SIZE * i)
            );
    }
...
vm_swap_in(swap_index_t swap_index, void *page){
    //Ensure thate the page is on user' s virtual memory.
    ASSERT(page >= PHYS_BASE);
    ...
    if(bitmap_test(swap_available, swap_index) == true){
        ...
        PANIC("Error, invalid read access to unsigned swap block");
    }
