...
page_fault (struct intr_frame *f)
{
   ...
   * First, bring in the page to which fault_addr refer
   */
  ...
  /* Obtain the current value of the user program's stack pointer. 
   * If the page fault is from user mode, we can obtain from intr_frame
   * 'f', but we cannot from kernel mode. We've stored the current esp
   ...
   */
   ...
   on_stack_frame = (esp <= fault_addr || fault_addr == f->esp - 4 ||
       fault_addr == f->esp - 32);
   ...
   if(on_stack_frame && is_stack_addr){
        ...
        * We need to add new page entry in the SUPT, if there was no page in
        * the SUPT. 
        * A promising choice is a assign a new zero-page. 
        */
       if(vm_supt_has_entry(cur->supt, fault_page) == false)
           vm_supt_install_zeropage(cur->supt, fault_page);
   }
