...
static int32_t get_user (const uint8_t *);
static int memread_user (void *, void *, size_t);
static void fail_invalid_access(void);
static struct file_desc *find_file_desc(struct thread *,int);
...
sys_exit(int status) {
  ...
  if(pcb != NULL){
      ...
      /* pcb = NULL probably means theat proviously
 ...
 * 'uaddr' must be below PHY_BASE
 * Return the byte value if successful(extract the least significant byte),
 * or -1 in case of error (a segfault occured or invalid uaddr)
 */
static int32_t
...
check_user(const uint8_t *uaddr){
    //check uaddr range or segfault
    if(get_user(uaddr) == -1)
        fail_invalid_access();
}

/* Write a single byte(content is 'byte') to user address 'udes'
 * 'udst' must be below PHYS_BASE
 * Return true if successful,false if a segfault occured
 */
static bool
